...
namespace Dapper
{
    ...
    public static partial class SimpleCRUD
    {
        ...
        /// <para>-Table name can be overridden by adding an attribute on your class [Table("YourTableName")]</para>
        /// <para>
        /// Deletes records where the Id property and properties with the [Key] attribute match those
        /// in the database
        /// </para>
        ...
        /// <para>-Table name can be overridden by adding an attribute on your class [Table("YourTableName")]</para>
        /// <para>
        /// conditions is an SQL where clause and/or order by clause ex: "where name='bob'" or "where age&gt;=@Age"
        /// </para>
        /// <para>
        /// parameters is an anonymous type to pass in named parameter values: new { Age = 15 }
        /// </para>
        ...
        public static TKey Insert<TKey>(this IDbConnection connection, object entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            if ((keytype == typeof(int) || keytype == typeof(long)) && Convert.ToInt64(idProps.First().GetValue(entityToInsert, null)) == 0)
            {
                ...
                {
                    sb.Append($" returning {GetColumnName(idProps.First())} into :newid");
                }
                else
                {
            ...
            // 使用ORACLE資料庫時，取得自增ID
            if (_dialect == Dialect.Oracle && keyHasPredefinedValue == false)
            {
                DynamicParameters param = new DynamicParameters(entityToInsert);

                param.Add(name: "newid", dbType: DbType.Int32, direction: ParameterDirection.Output);

                connection.Execute(sb.ToString(), param, transaction, commandTimeout);

                return param.Get<TKey>("newid");
            }
        ...
        public static void SetDialect(Dialect dialect)
        {
            switch (dialect)
            {
                case Dialect.Oracle:
                    ...
                    _encapsulation = "{0}";
                    _getIdentitySql = string.Empty;
                    _getPagedListSql = "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY {OrderBy}) AS PagedNumber, {SelectColumns} FROM {TableName} {WhereClause}) WHERE PagedNumber BETWEEN (({PageNumber}-1) * {RowsPerPage} + 1) AND ({PageNumber} * {RowsPerPage})";
                    _parameterPrefix = ":";
                    break;
        ...
        /// <para>-Table name can be overridden by adding an attribute on your class [Table("YourTableName")]</para>
        /// <para>
        /// Updates records where the Id property and properties with the [Key] attribute match those
        /// in the database.
        /// </para>
        /// <para>Properties marked with attribute [Editable(false)] and complex types are ignored</para>
        /// <para>Supports transaction and command timeout</para>
        ...
        /// <param name="connection"></param>
        /// <param name="entityToUpdate"></param>
        /// <param name="conditions">conditions is an SQL where clause ex: "where name='bob'"</param>
        /// <param name="transaction"></param>
        /// <param name="commandTimeout"></param>
        /// <returns>The number of effected records</returns>
        public static int Update(this IDbConnection connection, object entityToUpdate, string conditions, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            List<PropertyInfo> idProps = GetIdProperties(entityToUpdate).ToList();
        ...
        public class ColumnNameResolver : IColumnNameResolver
        {
            public virtual string ResolveColumnName(PropertyInfo propertyInfo)
            {
                ...
                dynamic columnattr = propertyInfo.GetCustomAttributes(true).SingleOrDefault(attr => attr.GetType().Name == typeof(ColumnAttribute).Name) as dynamic;
                if (columnattr != null)
                {
    ...
    /// <summary>
    /// Optional Table attribute. You can use the System.ComponentModel.DataAnnotations version in
    /// its place to specify the table name of a poco
    /// </summary>
